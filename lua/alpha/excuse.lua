...
local format_excuse = function(excuse, max_width)
    -- Converts list of strings to one formatted string (with linebreaks)
    formatted_excuse = " \n "  -- adds spacing between alpha-menu and footer
    for _, line in ipairs(excuse) do
        local formatted_line = format_line(line, max_width)
        formatted_excuse = formatted_excuse .. formatted_line
    end
    return formatted_excuse
end

local get_excuse = function(excuse_list)
    -- selects an entry from excuse_list randomly
    math.randomseed(os.time())
    local ind = math.random(1, #excuse_list)
    return excuse_list[ind]
end
...
{'Your mouse has moved. Please restart Windows for the change to take effect.'}
    ...
    local excuse = get_excuse(excuse_list)
    local formatted_excuse = format_excuse(excuse, max_width)
